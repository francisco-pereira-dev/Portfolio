...
acrescentaram — a 9 pôs o CV no site como página, a 11 publicou tudo, a 12
reestruturou o CV, a 14 fechou o projeto (a classe morta do CSS saiu, e o procedimento
para acrescentar o emprego de outubro à Experiência ficou escrito, na secção 9), a 15
suavizou o fade do menu, e a 16 pôs o README em inglês e arrumou a página do repositório
no GitHub, e a 18 passou o contrato para esta pasta. As regras, as decisões e o mapa
geral estão no [CLAUDE.md](CLAUDE.md), aqui ao lado.
...
### `npm run build` — hoje: exit 0, 0 avisos, 18 páginas, 45 ficheiros, 1 515,0 KB
...
**A publicar agora (fase 18, 2026-09-22):** "Normalise every text file". **Um só
commit**, por exigência do enunciado: leva o contrato para
`docs/CLAUDE.md`, deixa na raiz um `CLAUDE.md` com duas linhas que o importa, corrige os
caminhos que o movimento parte, e acrescenta uma linha vazia no fim de cada ficheiro de
texto versionado. **O hash não está aqui de propósito:** só existe depois do commit, e
registá-lo obrigava a um segundo commit, que é o que esta fase proíbe. Fica no
relatório técnico da fase seguinte, e no histórico do git.

O `git mv` foi feito pelo Francisco, à mão, no commit `e1452b2` das 14:11 — uma
renomeação pura, com o mesmo objeto do git dos dois lados (`5c4a5ec`). Esta fase apanhou
o trabalho já com o ficheiro em `docs/`, e fez o resto.

**A linha vazia no fim de cada ficheiro é cosmética, e foi pedida assim:** serve para a
listagem do GitHub mostrar o mesmo "último commit" em todos os ficheiros de texto. Não
entram os binários (PDF, imagens, fontes, favicon) nem o `public/CNAME`.

### A publicação anterior, `275d03d`

("Simplify the README to plain text and reword the repository
description"), publicado a 2026-09-22 às 12:51:17 UTC (13:51 em Lisboa), na fase 17. Um
commit, feito e enviado **numa sessão do Claude, com autorização expressa do
Francisco**, com seis ficheiros: o `README.md` só com texto, os dois relatórios da fase,
o `CLAUDE.md`, o `ESTADO-ATUAL.md` e o mapa. **Não muda o site:** o README não vai para
o `dist/`, e o que está no ar continua a ser o da fase 15. O commit seguinte, só de
documentação, fecha a fase 17.

**A Action correu verde**, pela ordem certa: `Verificar` das 12:50:22 às 12:50:38 UTC,
`Build` das 12:50:41 às 12:50:55, `Deploy` das 12:51:00 às 12:51:17.

**Confirmado na página do repositório**, logo a seguir: o README renderiza com as oito
secções e os dois blocos de código, **zero imagens**, e um só link para o site, na linha
"Live"; a descrição nova e os sete topics aparecem na barra lateral.

### A publicação anterior, `ad4a08d`

("Rewrite the README in English and tidy the repository page"),
publicado a 2026-09-22 às 12:34:55 UTC (13:34 em Lisboa), na fase 16. Um commit, feito e
enviado **numa sessão do Claude, com autorização expressa do Francisco**, com seis
ficheiros: o `README.md` refeito em inglês, os dois relatórios da fase, o `CLAUDE.md`, o
`ESTADO-ATUAL.md` e o mapa. **Não muda o site:** o README não vai para o `dist/`, e o
que está no ar continua a ser o da fase 15. O commit seguinte, só de documentação, fecha
a fase 16.

**A Action correu verde**, pela ordem certa: `Verificar` das 12:33:11 às 12:33:27 UTC,
`Build` das 12:33:30 às 12:33:43, `Deploy` das 12:33:47 às 12:34:55.

**Confirmado na página do repositório**, logo a seguir: o README renderiza com o badge
("Deploy to GitHub Pages — passing") e com a imagem de partilha, carregada a 1200×630
pelo caminho relativo, que o GitHub resolve para `raw/main/public/og-image.png`; a
descrição, o endereço do site e os sete topics aparecem na barra lateral.

### A publicação que está no ar, `1826977`

("Soften the overlay menu fade and honour reduced motion"),
publicado a 2026-09-22 às 12:08:47 UTC (13:08 em Lisboa), na fase 15. Um commit, feito e
enviado **numa sessão do Claude, com autorização expressa do Francisco**, com seis
ficheiros: o `global.css` com a curva do fade do menu e o menu no bloco de
`prefers-reduced-motion`, os dois relatórios da fase, o `CLAUDE.md`, o `ESTADO-ATUAL.md`
e o mapa. O `b43fbc9`, só de documentação, fechou a fase 15 sem mudar o site.

**A Action correu verde**, pela ordem certa: `Verificar` das 12:08:00 às 12:08:14 UTC,
`Build` das 12:08:16 às 12:08:30, `Deploy` das 12:08:36 às 12:08:47.

**Verificado contra o site ao vivo a seguir** (12:09 UTC): o sitemap com 18 páginas e as
**18 a responder 200**; o CSS partilhado servido com `transition: opacity .4s ease-out,
visibility 0s linear .4s` e o menu dentro do bloco de movimento reduzido. O site mudou
num só ponto: o nome do CSS partilhado (hash do conteúdo) e as três regras do menu lá
dentro.

### A publicação de 2026-09-22 de manhã, `3ecefe9`

("Remove the unused .cv-item-linha rules and document how to add an
Experience entry"), publicado a 2026-09-22 às 10:44:40 UTC (11:44 em Lisboa), na fase
14. Um commit, feito e enviado **numa sessão do Claude, com autorização expressa do
Francisco**, com quatro ficheiros: o `cv.css` sem as duas regras `.cv-item-linha`, o
`ESTADO-ATUAL.md` com a secção 9, o `CLAUDE.md` a apontar para ela, e o mapa. O
`3ac5da1`, só de documentação, fechou a fase 14 sem mudar o site.

**A Action correu verde**, pela ordem certa: `Verificar` das 10:43:52 às 10:44:09 UTC
(checkout, Node 24, dependências, build, paridade PT/EN e "O site diz o texto aprovado"),
`Build` das 10:44:11 às 10:44:27, `Deploy` das 10:44:31 às 10:44:40.

**Verificado contra o site ao vivo a seguir** (10:45 UTC), contra um build limpo do
`3ecefe9` feito de um `git archive` em `node_modules/.cache/`:

- `http://` → 301 para `https://`; certificado Let's Encrypt válido até 2026-10-27;
- o sitemap ao vivo com 18 páginas, e as 18 a 200;
- 44 ficheiros byte a byte iguais ao build limpo: as 18 páginas, os 2 do sitemap, os 2
  CSS, as 8 fontes, a fotografia, as 9 imagens de partilha, o favicon e os 2 PDF (43),
  e o `robots.txt`, que é byte a byte o objeto do git (`e73f0cf`; o export dá 4 bytes a
  mais por causa do `* text=auto` do `.gitattributes` nesta máquina Windows);
- num browser, 114 respostas nas 18 páginas, zero erros e zero 404.

O site mudou num só ponto: as duas páginas do CV apontam para `CvPage.CShgY8iK.css`
em vez de `CvPage.CxTYq8xd.css` (o Astro põe um hash do conteúdo no nome), e esse CSS é
o anterior menos as duas regras. As outras 16 páginas, as capturas do CV nos dois temas
e os dois PDF são byte a byte os de antes.

### A publicação de 2026-09-19, `207ed69` (e o `b211268`, só de documentação)

Publicado a 2026-09-19 às 00:01:46 UTC (01:01 em Lisboa). São dois
...
O `b211268`, só de documentação, fechou a fase 13 sem mudar o site: a Action dele correu
verde (`Verificar` das 00:05:28 às 00:05:43 UTC, `Build` até às 00:06:03, `Deploy` até
às 00:06:16), e a 2026-09-22 o site foi verificado de novo contra um build limpo dele
(18 páginas a 200; 43 ficheiros byte a byte e o `robots.txt` igual ao objeto do git;
1026 verificações do `check:texto` contra o HTML ao vivo; 114 pedidos, zero erros).

### A publicação antes dessa, `cb5e0dd`
...
- **Outubro de 2026: acrescentar o emprego novo à Experiência do CV**, pelo procedimento
  da secção 9. Com 8,9 mm de folga no PDF, vai ter de sair texto, e isso é decisão do
  Francisco; o candidato já identificado é o terceiro ponto do Licas (o dos cupões).
- **Para saber, não para fazer:** o PDF do CV tem 8,9 mm de folga — cerca de duas linhas.
  ...
  o diagnóstico antes de repetir. Na fase 14 correu limpo à primeira.

**Decisões do Francisco, que já não são pendentes:**

- **O README é em inglês (fase 16) e é só texto (fase 17).** Explica como o projeto
  funciona: o texto verificado antes de cada publicação, o PDF do CV gerado da página e
  reproduzível, as três guardas do build, o deploy travado pelas verificações, nada de
  terceiros em runtime e o site sem JavaScript. A fase 17 tirou-lhe o badge da Action e
  a imagem de partilha, encurtou-o de 4,2 KB para 2,8 KB, e deixou **um só link para o
  site**, na linha "Live". **A imagem `public/og-image.png` fica onde está:** é a
  pré-visualização social que o `Seo.astro` serve, e nunca teve nada a ver com o README.
  No GitHub ficaram a descrição, a homepage e sete topics. **O README não é texto do
  site e não passa pelo canónico**, mas as afirmações que faz sobre o projeto têm de
  continuar verdadeiras: são as mesmas que este documento regista.
- **O fade do menu overlay é de 0,4s com a curva `ease-out`** (fase 15; era `ease`). O
  Francisco escolheu entre três hipóteses — manter a duração e mudar só a curva,
  encurtar para 0,2s, ou alongar para 0,55s — e ficou a primeira. **A duração não se
  muda sozinha:** o atraso da `visibility`, que tira os links da ordem de tabulação, está
  acoplado a ela. O menu passou também a estar no bloco de `prefers-reduced-motion`,
  onde não tem transição nenhuma. Detalhes e medições no relatório da fase 15.
- **A classe `.cv-item-linha` saiu** (fase 14), com a prova de sempre e uma diferença
  aceite pelo Francisco: o Astro põe um hash do conteúdo no nome do CSS, por isso as
  duas páginas do CV mudaram nesse `href` e em mais nada. As outras 16 páginas, o CSS
  (o antigo menos as duas regras), as quatro capturas do CV nos dois temas e os dois
  PDF ficaram byte a byte iguais.
...
- **Fase 14:**
  - a prova da classe apagada feita com duas builds na cache, "antes" e "depois", quatro
    capturas de ecrã (as duas páginas do CV nos dois temas, a 1440px, página inteira) e
    os dois PDF regenerados; as capturas foram tiradas duas vezes na mesma build, para
    provar que são determinísticas;
  - quando a verificação "HTML byte a byte igual" falhou à letra (só no nome do CSS), o
    `cv.css` foi reposto a partir do blob do git e a pergunta feita antes de qualquer
    passo seguinte; a classe só voltou a sair depois da resposta;
  - o procedimento de outubro ficou como secção 9, numerada, no fim deste ficheiro, e o
    `CLAUDE.md` aponta-lhe da secção do CV;
  - o build limpo para a verificação ao vivo foi extraído com `git -c
    core.autocrlf=false archive`; mesmo assim o `robots.txt` sai com CRLF (é o
    `* text=auto` do `.gitattributes`), e a prova dele é contra o objeto do git;
  - o mapa foi refeito no mesmo commit da classe, porque o tamanho do `cv.css` e dos
    documentos mudou.
- **Fase 15:**
  - a duração do fade ficou nos 0,4s por causa do acoplamento à `visibility`, e a
    pergunta ao Francisco foi feita com as três hipóteses medidas, e não com uma
    proposta só;
  - o bloco de `prefers-reduced-motion` passou a cobrir o menu, e **só o menu**: o botão
    de três traços e a revelação de 0,8s do conteúdo continuam de fora, porque o
    enunciado mandava não tocar em mais nada;
  - a razão da curva ficou escrita no próprio CSS, com as percentagens medidas, para
    ninguém a trocar por distração.
- **Fase 16:**
  - o URL do LinkedIn no README é o do `src/data/site.json`, com a barra final, e não o
    do enunciado, que não a tinha — o enunciado mandava usar o do `site.json` se
    diferissem;
  - a descrição do README no mapa foi reescrita à mão, porque a que lá estava descrevia
    o README antigo (o tamanho ajusta-se sozinho, a descrição não);
  - as afirmações do README foram conferidas contra o projeto antes de escrever, e não
    copiadas às cegas: duas dependências de runtime, o Playwright e o axe só em
    desenvolvimento, e o Node 24 no workflow;
  - a descrição e os topics do repositório foram postos assim que se confirmou a
    permissão, antes de o texto do README chegar, porque não dependiam dele.
- **Fase 17:**
  - a descrição do README no mapa foi outra vez reescrita à mão, porque a da fase 16
    prometia um badge e uma imagem que deixaram de existir — o gerador do mapa corrige
    tamanhos, nunca afirmações;
  - confirmou-se que a `public/og-image.png` continua a ser pedida pelo `Seo.astro`
    antes de tirar o README de cima dela, para não ficar um ficheiro órfão sem que
    ninguém desse por isso.

## 9. Como acrescentar uma entrada à Experiência

Escrito na fase 14 para outubro de 2026, quando o Francisco começa num emprego novo e o
acrescenta ao CV. É o passo a passo real deste projeto, pela ordem. A regra do texto
canónico (`CLAUDE.md`) manda em tudo, e o `check:texto` e o `npm run cv` são quem prova
que foi cumprida.

1. **O texto entra no canónico**, em `cv.experiencia` do `content/texto-canonico.json`,
   copiado literalmente do enunciado em que o Francisco o aprovou, nas duas línguas, com
   a fonte da fase registada em `_sobre.fontes` (como `fase-12` e `fase-13`). Uma entrada
   tem os cinco campos das duas que lá estão: `funcao` (o título), `empresa`, `quando`
   (a data, com o traço " – " das outras), `tech` (a stack, com " · " entre as
   tecnologias) e `pontos` (a lista). **Cada ponto começa por um verbo no passado**
   ("Construí", "Built"), como manda a regra do `CLAUDE.md`.
2. **Gera-se o `src/data/cv.json`** a partir da secção `cv` do canónico, sem as chaves
   que começam por `_`, com dois espaços de indentação, e confirma-se que bate com o
   canónico. Não há comando para isto: cada fase o fez com um script temporário em
   `node_modules/.cache/`, e a prova é a comparação dos dois ficheiros.
3. **A entrada nova é a primeira da Experiência.** A lista está por ordem cronológica
   inversa (hoje: Licas, 2026; ITGlee, 2021), e a página e o PDF seguem a ordem do
   canónico.
4. **O `check:texto` tem de conhecer a entrada nova, pela ordem.** Ele lê
   `cv.experiencia` do canónico e compara posição a posição com o HTML das duas
   páginas — a `funcao`, a `empresa`, a `tech`, o `quando`, a quantidade de pontos e cada
   ponto —, e depois a sequência inteira do `<main>`. Não é preciso mexer no script se a
   entrada tiver os mesmos cinco campos: confirma-se que a contagem sobe (hoje são 1026
   verificações, 513 por língua) e que continua a zero divergências. A guarda 3 do `npm
   run cv` faz o mesmo com o texto do PDF (hoje 55 strings por língua) e também sobe.
5. **`npm run build` → `npm run cv`, e a guarda de uma página decide.** Se o CV deixar
   de caber, o `npm run cv` sai com código 1, diz quantos milímetros faltam, e não
   escreve nenhum dos dois PDF.
6. 🔴 **Hoje sobram 8,9 mm — cerca de duas linhas.** Uma entrada nova com título, stack,
   data e um ponto ocupa quatro ou cinco. **Vai ter de sair texto**, e isso é decisão do
   Francisco, não de quem executa. O candidato já identificado é o terceiro ponto do
   Licas (o dos cupões): é o mais longo dos três, e o segundo ponto já mostra decisão
   técnica. Nunca se aperta a impressão nem se aceita uma segunda página.
7. **O que sair vai para `retirado`** no canónico, na chave da fase, com o texto inteiro
   nas duas línguas e o motivo.
8. **`npm run build` → `npm run mapa`, as duas auditorias, e os dois relatórios.** A
   ordem completa é build → `check:i18n` → `check:texto` → cv → build → `audit:a11y` →
   `audit:teclado` → mapa (o mapa mede o `dist/`, por isso corre depois do último build).
   Os relatórios vão para `docs/historico/FASE-N-tecnico.md` e `FASE-N-resumo.md`.

**O que NÃO se faz:**

- **não se edita o PDF à mão** — os dois PDF saem só do `npm run cv`, são reproduzíveis,
  e o git só os vê mudados quando o texto muda;
- **não se escreve texto diretamente no `cv.json`** — ele é gerado do canónico, e o
  `check:texto` apanha qualquer diferença entre os dois;
- **não se mexe na fotografia** — é o mesmo ficheiro do hero, e fica sem perdas por
  decisão do Francisco na fase 13.
