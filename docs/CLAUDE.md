...
lido no início de cada sessão como se estivesse na raiz. Se é a primeira vez que vês
este projeto, lê isto até ao fim antes de mexer em qualquer coisa. O detalhe do
estado atual está em [ESTADO-ATUAL.md](ESTADO-ATUAL.md).
...
CLAUDE.md                     na raiz, só o importador: a linha @docs/CLAUDE.md e a
                              instrução de ler este ficheiro. O conteúdo não vive lá
docs/CLAUDE.md                este ficheiro — o contrato de trabalho do projeto: a regra
                              do texto canónico, as decisões já tomadas, os comandos e o
                              estado
...
- **O menu overlay abre e fecha com um fade de 0,4s e a curva `ease-out`** (fase 15; era `ease`). Com `ease` acelerava a meio do percurso e chegava quase opaco muito antes do fim; com `ease-out` arranca de imediato e trava no fim. **A duração não se muda sozinha:** o atraso da `visibility`, que tira os links da ordem de tabulação, está acoplado a ela e tem de mudar em sincronia — mais curto deixa os links tabuláveis depois de o menu desaparecer, mais longo corta o fade a meio. O menu está no bloco de `prefers-reduced-motion`, onde a transição passa a `none` e a visibilidade muda no mesmo instante.
...
- **Para acrescentar uma entrada à Experiência** (o emprego novo, em outubro de 2026),
  o passo a passo está na secção 9 do `docs/ESTADO-ATUAL.md`, "Como acrescentar uma
  entrada à Experiência": o texto entra no canónico, o `cv.json` gera-se dele, a entrada
  nova é a primeira, o `npm run cv` decide se cabe, e com 8,9 mm de folga **vai ter de
  sair texto, por decisão do Francisco**. Não se edita o PDF à mão, não se escreve no
  `cv.json` e não se mexe na fotografia.
...
- **Fases concluídas:**
  ...
  - 14: o lock do git apagado, a classe `.cv-item-linha` apagada com prova, o
    procedimento para acrescentar uma entrada à Experiência (secção 9 do
    `docs/ESTADO-ATUAL.md`), e a publicação;
  - 15: o fade do menu overlay suavizado (`ease` → `ease-out`, com a mesma duração) e
    posto no bloco de `prefers-reduced-motion`;
  - 16: o `README.md` refeito em inglês, e a descrição, a homepage e os sete topics do
    repositório postos no GitHub;
  - 17: o `README.md` simplificado — só texto, sem badge nem imagens, com um só link
    para o site;
  - 18: este ficheiro passou para `docs/CLAUDE.md`, a raiz ficou com um `CLAUDE.md` de
    duas linhas que o importa, e todos os ficheiros de texto levaram uma linha vazia no
    fim, para a listagem do GitHub mostrar o mesmo commit em todos.
...
- **Último commit publicado: `275d03d`** ("Simplify the README to plain text and reword
  the repository description"), fase 17, feito **numa sessão do Claude, com autorização
  expressa do Francisco** (2026-09-22 às 13:50, hora de Lisboa). A Action correu verde:
  `Verificar` das 12:50:22 às 12:50:38 UTC, `Build` das 12:50:41 às 12:50:55 e `Deploy`
  das 12:51:00 às 12:51:17. **Não muda o site** — o README não vai para o `dist/`.
  Confirmado na página do repositório: o README renderiza sem imagens e com um só link
  para o site, e a descrição nova e os sete topics aparecem. Antes dele, o `ad4a08d`
  (fase 16), com o README em inglês.
- **O site em franciscopereira.dev corresponde ao commit `1826977`**
  ("Soften the overlay menu fade and honour reduced motion"), fase 15, publicado a
  2026-09-22 às 12:08:47 UTC. **Verificado contra o site ao vivo:** as 18 páginas
  a 200, e o CSS partilhado servido já com `opacity .4s ease-out` e o menu no bloco de
  movimento reduzido.
- **Publicação anterior:** o commit **`3ecefe9`**
  ("Remove the unused .cv-item-linha rules and document how to add an Experience
  entry"), fase 14, publicado a 2026-09-22 às 10:44:40 UTC, seguido do `3ac5da1`, só de
  documentação. **Verificado contra o site ao vivo:** `http://` → 301; as 18 páginas a
  200; 44 ficheiros byte a byte iguais a um build limpo do commit (o `robots.txt` é o
  objeto do git); 114 respostas num browser, zero erros e zero 404; certificado válido
  até 2026-10-27.
- **Publicação anterior:** o commit **`207ed69`**
  ("Replace an anglicism in the CV, add the Phase 11 reports, update the docs"),
  precedido de `1a55793` (o CV reestruturado, fase 12) e seguido do `b211268`, só de
  documentação. Os dois commits e o push foram
  ...
  nas duas línguas; certificado válido até 2026-10-27.
- **Antes dessa:** o commit **`cb5e0dd`**
...
- **Por fazer** (detalhe na secção "Por fazer" do `docs/ESTADO-ATUAL.md`):
  - em outubro de 2026, o emprego novo na Experiência do CV, pelo procedimento da
    secção 9 do `docs/ESTADO-ATUAL.md` — vai ter de sair texto, por decisão do
    Francisco;
